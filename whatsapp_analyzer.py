...
def parse_whatsapp_chat(text):
    ...
    # Patrón para las líneas del chat
...
def find_love_expressions(df):
    ...
    for idx, row in df.iterrows():
        ...
        if re.search(r'te quier[o\w]*', msg, re.IGNORECASE):
        ...
        # Te amo (y variantes)
        if re.search(r'te am[o\w]*', msg, re.IGNORECASE):
        ...
        if re.search(r'me encant[a\w]*', msg, re.IGNORECASE):
        ...
        # Amor (general)
        if re.search(r'\bamor\b', msg, re.IGNORECASE):
            results['amor'].append({
                'date': row['date'],
                'sender': row['sender'],
                'message': row['message'],
...
def find_palabra_del_dia(df):
    ...
    palabras = []
    pattern = r'(?:la\s+)?palabra\s+(?:del\s+)?día\s+(?:es|de)\s+(\w+)\s*,\s*que\s+es\s+([^\.]+\.)?' 
...
st.subtitle("Análisis de expresiones de amor y palabras del día")
        ...
        if len(df) == 0:
            ...
            with col1:
                ...
                st.metric("Te Quiero", count_tq, 
                         delta=f"❤️ desde {love_expr['te_quiero'][0]['date'].strftime('%d/%m') if love_expr['te_quiero'] else 'N/A'}")
            ...
            if love_expr['te_quiero']:
                ...
                te_quiero_timeline = te_quiero_df.groupby('date_only').size() if 'date_only' not in te_quiero_df.columns else te_quiero_df.groupby(te_quiero_df['date'].dt.date).size()
                
                # Recalcular con date correcta
                ...
                ax.fill_between(range(len(te_quiero_timeline)), te_quiero_timeline.values, alpha=0.3, color='#667eea')
                ax.set_xlabel("Fecha")
